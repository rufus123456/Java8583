...
public class Iso8583Message {
    ...
    public Iso8583Message setValue(int index,String value){
        ...
        //将数据填入map
        values.put(index,new Iso8583Field(index,value,type));
        //位图标记
        bitmap[index-1] = 1;
        return this;
    }
     ...
     * @Title: getValueFilled
     * @Description: 获取报文中的某个域的值
     * @param index 域的索引值
     * @return Iso8583Field 已填充值
     * @author Ajsgn@foxmail.com
     * @date 2017年3月27日 下午12:33:53
     */
    public Iso8583Field getValueFilled(int index){
        Iso8583Field field = getValue(index);
        Iso8583FieldType type = factory.getFieldType(index);
        String value = getFieldValueFilled(field.getValue(), type);
...
//              //使用例外的数据类型
            ...
            switch (type.getFieldTypeValue()) {
                case CHAR:{
                    ...
                    value = getFieldValueFilled(value, type);
    ...
    private byte[] numericVariableLengthParse(String value,int dataLength,boolean isByteLength,Iso8583FieldType type){
        ...
        int length = (type.getFillBlankStrategy().isLengthWithBlank() ? getFieldValueFilled(value, type).length() : value.length());
    ...
    /**
     * 根据域定义的要求，对用户输入的值做填充/补位
     * @Title: getFieldValueFilled
     * @Description: 获取某一个字段值的填充结果
     * @param value 原内容
     ...
     */
    private String getFieldValueFilled(String value,Iso8583FieldType type){
        if(Iso8583FieldType.FieldTypeValue.LLVAR_NUMERIC == type.getFieldTypeValue() 
            ...
            //获取补齐结果与非补齐结果报文内容
            String filledResult = value;           //填充的结果
            if(0 != value.length() % 2){
                //如果长度不为偶数，则需要填充
                //判断对齐放向
            ...
            return filledResult;
        }else if (Iso8583FieldType.FieldTypeValue.NUMERIC == type.getFieldTypeValue()
            || Iso8583FieldType.FieldTypeValue.BYTE_NUMERIC == type.getFieldTypeValue()
            || Iso8583FieldType.FieldTypeValue.CHAR == type.getFieldTypeValue()
            ){
            Iso8583FillBlankStrategy fillBlankStrategy = type.getFillBlankStrategy();
            int len = type.getFieldLength();
            if(len>value.length()) {
                String fillChar = StringUtil.repeat(String.valueOf(fillBlankStrategy.getValue()), len-value.length());
                return fillBlankStrategy.isLeftAppend() ? fillChar + value : value + fillChar;
            }else {
                return value;
            }            
        }else{
